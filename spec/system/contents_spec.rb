...
RSpec.describe "コンテンツ投稿機能", type: :system do
  ...
  context 'コンテントを投稿できるとき' do
    it 'ログインしたお仕事をされている大人（contributor）は、コンテンツを投稿できる' do
      ...
      # （添付する動画を定義する）
      # 動画選択フォームに動画を添付する
      attach_file('content[video_content]', 'spec/fixtures/test.MOV')
      ...
      expect(page).to have_selector("video[src*='test.MOV']")
...
RSpec.describe 'コンテンツ削除機能', type: :system do
  ...
  context 'コンテンツ削除ができるとき' do
    it 'ログインした仕事をされている大人contributorは自らが投稿したコンテンツの削除ができる' do
      ...
      # expect {
      #   find_link('削除', href: content_path(@content1)).click
      # }.to change { Content.count }.by(-1)
      # 削除が完了するとトップページへ遷移する
      # expect(current_path).to eq root_path
      ...
      # expect(page).to have_no_selector("video[src*='test.MOV']")
